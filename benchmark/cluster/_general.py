...
def generally_cluster_obs(
    ...
    if isinstance(cl_method, str):
        ...
        cl_data = read_clusters_from_cache(data_name, cl_method, run)
        if cl_data is None:
            n_clusters = k
            cl_data = cluster_genes(adata, img, species, cl_method, n_clusters, random_state=random_state+run)
            write_clusters_as_cache(cl_data, data_name, cl_method, run)
    elif callable(cl_method):  # cl_method is a custom functions
        cl_data = read_clusters_from_cache(adata, cl_method.__name__, run)
        if cl_data is None:
